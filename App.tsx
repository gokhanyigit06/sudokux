...
function App() {
  ...
  const handleHint = () => {
    if (isPaused || isComplete) {
      console.log('Hint blocked: isPaused=' + isPaused + ', isComplete=' + isComplete);
      return;
    }
    
    const {getHint} = require('./src/utils/sudoku');
    const boardValues = board.map(r => r.map(c => c.value));
    console.log('Board values for hint:', JSON.stringify(boardValues));
  ...
  const handleChangeDifficulty = () => {
    console.log('handleChangeDifficulty called');
